...
plot1 <- 
  ggplot(data = long_run_df) + 
  geom_histogram(mapping = aes(x = age, fill = sex), binwidth = 1) +
  geom_density(mapping = aes(x = age, y = after_stat(count))) +
  facet_wrap(vars(sex)) +
  labs(title = "Breakdown of Runner Age by Sex", 
...
`ggplot2` allows you to update all future figures by setting a theme, without needing to pass in new arguments.

Try reproducing the above plot but with the pre-made theme called `theme_dark()`.
...
A common technique for exploring relationships is called a scatter matrix, which scatters each variable against another. We can also assign colours based on categorical attributes such as sex.

```{r}
# note that theme_dark() set above is still active

library(GGally)
ggscatmat(data = long_run_df, 
          columns = c(1,4,6), alpha = 0.1, color = "sex")
```
...
2. Discuss what other visualisations might you make (using `plot()` or `ggplot2`) to explore relationships in the data? How might you condition or display the data beforehand?
