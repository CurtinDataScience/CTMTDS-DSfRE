...
This table contains a range of recipes for concrete – including cement, water, aggregate, and age; and the resulting compressive strength. Compressive strength is the "response variable", and the recipe amounts are the "explanatory variables".
...
- Use the `head()` function to print to screen the first few rows (6 by default).
- Type out the name of the object in a code chunk and RStudio will automatically displays its content when the chunk is executed.
...
# median: concrete$...
median(concrete$CompressiveStrength)
```
...
# concrete...
quantile(concrete$CompressiveStrength, probs = c(0.25, 0.75))
```
...
# concrete..
concrete_modified <- concrete # make a copy of the data frame first
concrete_modified[, "Age"] <- concrete_modified[, "Age"] + 2
concrete_modified
```
...
```{r}
mask <- chemical$ManufacturingProcess38 == 2
chemical[mask,]
...
yield2 <- colMeans(subset(chemical, subset = ManufacturingProcess38 == 2, select = Yield))
yield3 <- colMeans(subset(chemical, subset = ManufacturingProcess38 == 3, select = Yield))
...
The `%in%` operator returns `TRUE` if the value mentioned before it is found in the range specified after it.
