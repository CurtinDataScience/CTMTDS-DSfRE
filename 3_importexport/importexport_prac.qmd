...
```{r}
backblaze <- read_parquet("../data/backblaze_event.parquet")
```
...
Filter rows using the `subset()` function and passing in an expression that evaluates to TRUE for the rows that you want.

You can specify which columns to return using the `select =` argument.
...
# 7. Advanced: Setting the row names
...
We can show these "row names" in chunk outputs by inserting `rownames.print=TRUE` into the preamble of the code chunk found in the curly braces.
...
However, in practice, it is recommended to filter rows by their row number directly or by the values in the other columns. Alternatively, create a new column which is the row number and filter according to that.
