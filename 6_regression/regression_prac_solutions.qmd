...
ore_df <- ore_df[order(ore_df$Amount),] # sorts rows in ascending `Amount`
str(ore_df)
```
...
```{r}
plot(Absorbance ~ Amount, data = ore_df)
```
...
```{r}
plot(sqrt(Absorbance) ~ Amount, data = ore_df)
```

```{r}
# plot(log(...) ~ ..., data = ...)
plot(log(Absorbance) ~ Amount, data = ore_df)
```

```{r}
# Another plot()
plot(Absorbance^(1/3) ~ Amount, data = ore_df)
```

```{r}
# Another plot()
plot(Absorbance^2 ~ Amount, data = ore_df)
```

```{r}
# Another plot()
plot(Absorbance^3 ~ Amount, data = ore_df)
```
...
# summary(model6)
ore_model <- lm(Absorbance^(1/3) ~ Amount, data = ore_df)
summary(ore_model)
```
...
```{r}
plot(Absorbance^(1/3) ~ Amount, data = ore_df)
lines(ore_df$Amount, ore_model$fitted.values, col = "red")
```
...
plot(Absorbance ~ Amount, data = ore_df, main = "Ore Model")
lines(ore_df$Amount, fitted_values, col = "blue", lwd = 3)
polygon(x = c(ore_df$Amount, rev(ore_df$Amount)), 
