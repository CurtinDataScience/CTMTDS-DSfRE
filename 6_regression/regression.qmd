...
* What we'd like to have is a confidence interval for the failure time; in the engineering literature, graphically extrapolating the intervals to the time axis is common, but ...
* In reliability analysis, we ask the question, "What is the probability of failure (however defined) at some time $t$ and what is the interval for the _probability_?"
  * Can do so by calculating the _failure time distribution_ (advanced topic!)
:::
:::
