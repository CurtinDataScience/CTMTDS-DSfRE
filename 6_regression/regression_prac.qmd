...
points(x = concrete_df$Cement, y = model$fitted.values, col = "red")
```
...
plot(x = concrete_df$Cement, y = model$residuals, col = "brown",
...
qqline(model$residuals, col = "magenta", lwd = 2)
```
...
```{r}
model <- lm(CompressiveStrength ~ BlastFurnaceSlag, data = concrete_df)

plot(formula(model), data = concrete_df)
abline(a = model$coefficients[1], b = model$coefficients[2], 
       col = "blue", lwd = 2)
```
...
```{r}
# Interaction terms: use "y ~ (a + b + c)^2", but replace y, a, b, c, with the variables in our data frame

```

## 3.2 Assessing the goodness of fit

Now have a look at the output of `summary(model)`.
...
lines(ore_df$Absorbance, model$fitted.values, col = "red")
```
...
# pi_lwr = predictions[,"lwr"]...
# pi_upr = predictions[,"upr"]...
...
polygon(x = c(ore_df$Absorbance, rev(ore_df$Absorbance)), 
        ...
        col = rgb(0, 0, 1, 0.2), lty = 0) # transparent blue
```
