...
concretefiltered <- 
  concrete[(concrete$CompressiveStrength > quartiles[2] - 3*iqr) 
           & (concrete$CompressiveStrength < quartiles[2] + 3*iqr),]
```

Finally, let's visualise our dataset again using a box plot to confirm the results of our outlier removal:

```{r}
boxplot(concretefiltered$CompressiveStrength, 
        horizontal = TRUE, 
        xlab = "Compressive Strength")
```
...
```{r}
outlier_indices <- which(chemical$Yield > 45) # visually identified from the boxplot above
chemical_cleaned <- chemical[-outlier_indices,]

boxplot(chemical_cleaned$Yield,
        horizontal = TRUE, 
        xlab = "Yield")
```
...
quartiles <- quantile(chemical$Yield, probs = c(0.25, 0.5, 0.75))
iqr <- quartiles[3] - quartiles[1]

chemical_filtered <- 
  chemical[(chemical$Yield > quartiles[2] - 3*iqr) 
           & (chemical$Yield < quartiles[2] + 3*iqr),]

boxplot(chemical_filtered$Yield,
        horizontal = TRUE, 
        xlab = "Yield")
```
...
# concrete2 <- read.csv("...")
# concrete2
concrete2 <- read.csv("../data/concrete-2.csv")
```
...
```{r}
# models <- read.csv(...)
models <- read.csv("../data/backblaze-models.csv")
models
```
...
```{r}
# merge(x = ..., y = ..., by.x = "...", by.y = "...")
merge(x = backblaze, y = models, by.x = "model", by.y = "model")
```
...
# merge(x = ..., y = ..., by.x = "...", by.y = "...")
merge(x = backblaze, 
      y = models[c("model", "operating_power")], 
      by.x = "model", 
      by.y = "model")
```
...
grouping <- as.Date(cut(backblaze$date, "months"))
monthly <- aggregate(x = backblaze$failure, by = list(Date = grouping), FUN = sum)

plot(monthly, 
     type = "h", 
     main = "Monthly hard drive failures", 
     ylab = "No. failures")
```
...
backblaze_oppower <- merge(x = backblaze, 
                           y = models[c("model", "operating_power")], 
                           by.x = "model", 
                           by.y = "model")

grouping <- as.Date(cut(backblaze_oppower$date, "7 day"))
seven_daily <- aggregate(x = backblaze_oppower$operating_power, 
                         by = list(Date = grouping), 
                         subset = backblaze_oppower$failure == 1,
                         FUN = "sum")

plot(seven_daily,
     type = "h", 
     main = "7-day failed hard drive operating power", 
     ylab = "Power")
```
